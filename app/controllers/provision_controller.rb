class ProvisionController < ApplicationController
  ...
  def show
    ...
    @candidates = Candidate.joins(:candidate_votes).select("candidates.*, COUNT(candidate_votes.candidate_id) AS vote").where("provision_id = ?", params[:id]).group("candidate_votes.candidate_id")
  end
